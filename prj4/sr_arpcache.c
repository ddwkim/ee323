...
void sr_arpcache_handle_arpreq(struct sr_instance *sr, struct sr_arpreq *req) {
  ...
  */
  ...
  struct sr_icmp_t3_hdr *ic3_hdr;           /* ICMP type3 header */
  ...
  if (difftime(curtime, req->sent) > 0.5) {
    ...
    if (req->times_sent >= 5) {
      /* send ICMP host unreachable to source addr of all pkts waiting on this
       * request */
      ...
      for (pck = req->packets; pck != NULL; pck = pck->next) {
        ...
        i_hdr0 =
            (struct sr_ip_hdr *)(pck->buf + sizeof(struct sr_ethernet_hdr));
        ...
        i_hdr->ip_src = ifc->ip;
        i_hdr->ip_sum = 0;
        i_hdr->ip_sum = cksum(i_hdr, sizeof(struct sr_icmp_t3_hdr));

        /* ICMP header, type 3, code 1 */
        ic3_hdr =
            (struct sr_icmp_t3_hdr *)(buf + sizeof(struct sr_ethernet_hdr) +
                                      sizeof(struct sr_ip_hdr));
        ic3_hdr->icmp_type = 0x03;
        ic3_hdr->icmp_code = 0x01;
        ic3_hdr->icmp_sum = 0;
        ic3_hdr->unused = 0;
        ic3_hdr->next_mtu = 0;
        memcpy(ic3_hdr->data, i_hdr0, ICMP_DATA_SIZE);
        ic3_hdr->icmp_sum = cksum(ic3_hdr, sizeof(struct sr_icmp_t3_hdr));
        ...
        if (arpentry != NULL) {
          ...
          free(arpentry);
          /* send */
          PRINT_HEADER(buf, len, __LINE__);
          sr_send_packet(sr, buf, len, rtentry->interface);
        } else {
          /* queue */
          PRINT_DEBUG("ICMP host unreachable %d: queue\n", __LINE__);
          sr_arpcache_queuereq(&(sr->cache), i_hdr->ip_dst, buf, len,
                               rtentry->interface);
        }
      }
