...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc, argv, ":qU")) != EOF) {
    switch (opt) {
      case 'f':
        filename = optarg;
        break;
 ...
 *  -1 on failure
 */
static int process_line(int sd, char *line) {
  char resp[5000];
  int fd = -1, length;
...
void loop_until_end(int sd) {
  ...
  for (;;) {
    ...
    if (filename == NULL) {
      ...
      while ((pline > line - 1) && isspace((int)(*pline))) --pline;
...
int parse_address(char *address, struct sockaddr_in *sin) {
  ...
  if (NULL == (address_copy = strdup(address))) goto cleanup;
...
cleanup:
  if (address_copy) free(address_copy);
